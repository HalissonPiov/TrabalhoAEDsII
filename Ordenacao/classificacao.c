...
// Função para extrair o menor elemento da memoria, que estará no topo
TProduto extrairMinProduto(TProduto memoria[], int *n)
{
    TProduto min = memoria[0];
    memoria[0] = memoria[(*n) - 1];
    (*n)--;
    memMinimaProduto(memoria, *n, 0);
    return min;
}

// Função para inserir um elemento na memoria
void inserirMemoriaProduto(TProduto memoria[], int *n, TProduto novo)
{
    int i = (*n);
    memoria[i] = novo;
    (*n)++;
    ...
    while (i > 0 && memoria[(i - 1) / 2].id > memoria[i].id)
    {
        trocarProduto(&memoria[i], &memoria[(i - 1) / 2]);
        i = (i - 1) / 2;
    }
}
...
int selecaoNaturalProduto(FILE *entrada, int M)
{
    ...
    for (int i = 0; i < M; i++)
    {
        TProduto *produto = leProduto(entrada);
        if (produto)
        {
            memoria[tamanhoMemoria++] = *produto;
            printf("Carregado na memoria: ID = %d\n", produto->id);
            free(produto);
        }
        else
    ...
    for (int i = tamanhoMemoria / 2 - 1; i >= 0; i--)
    {
        memMinimaProduto(memoria, tamanhoMemoria, i);
    }
    ...
    while (!fimDeArquivo || tamanhoMemoria > 0)
    {
        ...
        if (tamanhoMemoria > 0)
        {
            TProduto menor = extrairMinProduto(memoria, &tamanhoMemoria);
            salvaProduto(&menor, saida);
            printf("Menor registro extraído e salvo na partição: ID = %d\n", menor.id);

            // Passo 3: Ler um novo registro do arquivo
            TProduto *novoRegistro = leProduto(entrada);
            if (novoRegistro)
            {
                ...
                {
                    comparacoes++;
                    inserirMemoriaProduto(memoria, &tamanhoMemoria, *novoRegistro);
                    printf("Novo registro inserido na memoria: ID = %d\n", novoRegistro->id);
                }
                else
                {
        ...
        if (tamanhoMemoria == 0 && tamanhoReservatorio > 0)
        {
            ...
            for (int i = 0; i < tamanhoReservatorio; i++)
            {
                inserirMemoriaProduto(memoria, &tamanhoMemoria, reservatorio[i]);
            }
            tamanhoReservatorio = 0;
        }
    }
...
void verificaParticoesProduto(int numParticoes)
{
    ...
    for (int i = 1; i <= numParticoes; i++)
    {
        ...
        while ((produto = leProduto(arquivo)))
        {
            printf("ID = %d\n", produto->id);
            if (anterior > produto->id)
            {
                ordenado = 0; // Se encontrar um valor fora de ordem, marca como erro
            }
            anterior = produto->id;
